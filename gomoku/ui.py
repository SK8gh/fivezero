...
# module imports
from gomoku.game import BOARD_SIZE, coordinates
...
def _result(game) -> str:
    ...
    """
    if game.winner == Colors.BLACK:
        return "Black wins"

    elif game.winner == Colors.WHITE:
        return "White wins"
...
def draw_game(surface, fonts, game, status, hover_idx, current_color, thinking=False, hint=None) -> None:
    ...
    if hover_idx is not None:
        ...
        body = STONE_B if current_color == Colors.BLACK else GHOST_W
...
class Menu:
    ...
    def _compute_regions(self, fonts) -> dict[str, pygame.Rect]:
        ...
        if self.dev_mode:
            ...
            regions["play_ava"] = fonts["mode"] \
                .render("AI vs AI", True, TEXT) \
                .get_rect(center=(cx, self._STONES_Y))

        else:
            # color stones
            regions["color_black"] = pygame.Rect(
                cx - self._GAP - R,
                self._STONES_Y - R,
                2 * R,
                2 * R
            ...
            regions["color_white"] = pygame.Rect(
                cx + self._GAP - R,
                self._STONES_Y - R,
                2 * R,
                2 * R
            ...
            regions["play_hva"] = s_ai.get_rect(
                midleft=(left, self._BUTTONS_Y)
            )
    ...
    def _stone(self, surface, center, color, selected, hovered) -> None:
        ...
        body = STONE_B if color == Colors.BLACK else STONE_W
        rim = STONE_W_E if color == Colors.WHITE else None
...
def _pill(surface, fonts, rect, label, active, mouse, font_key="small") -> None:
    ...
    if active:
        ...
        _round_rect(
            surface,
            rect,
            (236, 233, 227) if hovered else (244, 242, 237),
            rect.height // 2
        )
...
class AvaMenu:
    ...
    def selection(self):
        ...
        return (
            MODEL_CATALOG[self.a_idx],
            MODEL_CATALOG[self.b_idx],
            int(self.time_text),
            int(self.games_text)
        )
...
def draw_arena_hud(surface, fonts, arena) -> None:
    ...
    _filled_circle(
        ...
        panel.centery,
        6,
        STONE_B if a_black else STONE_W
    )
    ...
    _filled_circle(
        ...
        panel.centery,
        6,
        STONE_W if a_black else STONE_B
    )
